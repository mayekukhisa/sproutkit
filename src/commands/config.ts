...
import { config, flattenConfig } from "@/config"
import { performCustomValidation, validateKey } from "@/validators"

/**
 * Creates the config command with all subcommands.
 *
 * Provides four configuration management operations:
 * - set: Set a configuration value with validation
 * - list: Display all configuration settings
 * - delete: Remove a configuration key
 * - reset: Clear all configuration with confirmation
 *
 * @returns Commander.js Command instance for the config command
 */
export function createConfigCommand(): Command {
  const configCommand = new Command("config").description("Manage tool configuration settings")
  ...
  configCommand
    ...
    .action((key: string, value: string) => {
      try {
        validateKey(key)
        performCustomValidation(key, value)
  ...
  configCommand
    ...
    .action((key: string) => {
      try {
        ...
        // Type assertion needed because key is a runtime string value
        // but TypeScript expects a literal union of valid config keys
