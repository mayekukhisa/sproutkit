...
TBD - created by archiving change add-config-subcommand. Update Purpose after archive.
...
- **THEN** the path SHALL be stored as a template source
- **AND** the path SHALL be validated to exist

#### Scenario: Configuring remote template source

- **WHEN** user sets `templates.sources.remote` to a valid URL
- **THEN** the URL SHALL be stored as a template source
- **AND** the format SHALL be validated as a valid URL
